...
```bash
python scripts/wiki_graph_extract.py docs/wiki/ --out docs/wiki/graph
```
...
2. Run `python scripts/wiki_graph_lint.py docs/wiki/` — catches unknown predicates, broken object references, missing evidence, alias collisions.
3. Run `python scripts/wiki_graph_extract.py docs/wiki/ --out docs/wiki/graph` — regenerates the artifacts above.
4. Query with `python scripts/wiki_graph_query.py docs/wiki/ neighbors --node product:konvy` (or `edges`, `path`, `facts`).
